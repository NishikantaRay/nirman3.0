...
$(document).ready(function () {
  ...
  if (masterCurtain.length) {
    ...
    effectsMasterDelay =
      masterCurtainDelay + masterCurtainDuration + masterCurtainStagger * 2;
    ...
    //   ease: "power2.in",
    // });
    gsap.to(".mcurtain-right", {
      yPercent: 100,
      duration: masterCurtainDuration,
      delay: masterCurtainDelay - 0.8,
      stagger: masterCurtainStagger,
      ease: "power2.in",
    });
    gsap.to(".mcurtain-center", {
      yPercent: -100,
      duration: masterCurtainDuration,
      delay: masterCurtainDelay - 0.5,
      stagger: masterCurtainStagger,
      ease: "power2.in",
    });
    gsap.to(".mcurtain-left", {
      yPercent: -100,
      duration: masterCurtainDuration,
      delay: masterCurtainDelay - 0.7,
      stagger: masterCurtainStagger,
  ...
  function fadeInBasic() {
    ...
    while (n < max + 1) {
      ...
      $(elements).each(function (i) {
        ...
        let finalTarget = $(
          "." + expression + n + '[data-sk-fade-match="' + (i + 1) + '"]'
        );
        ...
        if (phase1Regular[n].length) {
          gsap
            ...
            .fromTo(
              phase1Regular[n],
              { opacity: 0, yPercent: 120 },
              { opacity: 1, yPercent: 0, duration: 0.4, stagger: 0.3 }
            );
        }
      ...
      while (n < numberOfHeroSlides + 1) {
        ...
        if (phase1[n].length) {
          gsap
            ...
            .to(
              phase1[n],
              { opacity: 0, yPercent: 120, delay: 0.15, duration: 0.5 },
              "<"
            );
        }
        ...
        if (phase2[n].length) {
          gsap.to(
            phase2[n],
            { opacity: 0, xPercent: 20, delay: 0.15, duration: 0.5 },
            "<"
          );
        }
    ...
    if (strips.length) {
      ...
      while (n < numberOfStrips) {
        if (n > 0) {
          $(parent)
            .find(
              ".sk__hero-parallax-strip-vertical:nth-child(" + (n + 1) + ")"
            )
            .css("left", width * n + "vw");
        }
        n++;
  ...
  function warpedText() {
    ...
    if (warpTextSection.length) {
      if (isMobile != true) {
        $(".sk__cta-warp:not(.not-animated) .sk__warped-text-wrapper").each(
          function () {
            let wrapper = $(this);
            // create clones
            let classnameL = "";
  ...
  $(".sk__gallery-basic-thumbnails").each(function () {
    ...
    if (numb >= 1) {
      $(gallery)
        .find(".carousel")
        .append('<div class="carousel-indicators sk__flex-center"></div>');
      ...
      if (
        ...
        let phsInfoBoxTL = gsap
          ...
          .fromTo(
            phsInfoBoxH4,
            { opacity: 0, y: -100 },
            { opacity: 1, y: 0, duration: 0.65, ease: "power1.out" },
            "-=1.2"
          );
      }
      // main heading
  ...
  gsap.utils
    ...
    .forEach((section, i) => {
      ...
      if (i) {
        ...
        section.bg.style.backgroundPosition = `${parallaxBackgroundOriginX} ${
          innerHeight / 2
        }px`;
        gsap.to(section.bg, {
          backgroundPosition: `${parallaxBackgroundOriginX} ${
            -innerHeight / 2
          }`,
          ease: "none",
          scrollTrigger: {
  ...
  if (isMobile != true) {
    ...
    var parallaxLayersSceneAbout = document.getElementById(
      "sk__parallax-layers-about"
    );
